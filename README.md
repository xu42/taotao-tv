...
按**返回**弹出居中的**设置面板**（画质 / 收藏当前频道 / 退出应用），
面板打开后 **1.2 秒内再按一次返回**即直接退出应用。
...
12. **平板适配** — 切台菜单只占左半边、右侧留白；触屏左半屏呼出菜单、右半屏呼出设置面板。
13. **退出交互重做** — 设置面板垂直水平居中、近黑圆角卡片，焦点按钮反白；连按两次返回键直接退出应用。
